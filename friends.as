package {
  ...
  public class Friends {
    ...
    public function add(uid:String, name:String, is_online:Boolean = false, world:String = "", rank:String = "", can_join:Boolean = false, is_request:Boolean = false, can_accept:Boolean = false, can_invite:Boolean = false, team_pvp_enabled:Boolean = false, is_ignored:Boolean = false, highlight:Boolean = false) : void {
      ...
      if(config.alts[friend.uid]) this.list_alts.push(friend);
      ...
      // for each (var color:String in this.colors) {
      //   if(config.colors[color][friend.uid]) {
    ...
    public function clear() : void {
      ...
      // for each (var color:String in this.colors) this.list_colors[color].splice(0);
    ...
    private function buildHeaderSection() : void {
      ...
      for (var i:int = 0; i < 7; i++) {
        ...
        this.color_btns.push(new ColorBtn(5, colors[i], new_x, 30));
        this.color_btns[i].toggled = config.cfg.active_color == colors[i];
        this.color_btns[i].addEventListener(MouseEvent.CLICK, this["changeColor" + colors[i].charAt(0).toUpperCase() + colors[i].slice(1)]);
      }
    ...
    public function onQuickListAdd(f:Friend) : void {
      ...
      if(this.list_colors[color].indexOf(f) == -1) this.list_colors[color].push(f);

      var debug3:TextField = renderer.text(368, 80, TEXT_FORMAT_ONLINE, "left", true, "list updated i guess");
      debug3.textColor = renderer.WHITE;
      this.full_list.addChild(debug3);

      // config.configWrite("colors");
    }
    ...
    private function changeColorRed() : void {
      ...
      for (var i:int = 0; i < 7; i++) if (i != 0) this.color_btns[i].toggled = false;
    ...
    private function changeColorOrange() : void {
      ...
      for (var i:int = 0; i < 7; i++) if (i != 1) this.color_btns[i].toggled = false;
    ...
    private function changeColorYellow() : void {
      ...
      for (var i:int = 0; i < 7; i++) if (i != 2) this.color_btns[i].toggled = false;
    ...
    private function changeColorGreen() : void {
      ...
      for (var i:int = 0; i < 7; i++) if (i != 3) this.color_btns[i].toggled = false;
    ...
    private function changeColorCyan() : void {
      ...
      for (var i:int = 0; i < 7; i++) if (i != 4) this.color_btns[i].toggled = false;
    ...
    private function changeColorBlue() : void {
      ...
      for (var i:int = 0; i < 7; i++) if (i != 5) this.color_btns[i].toggled = false;
    ...
    private function changeColorPurple() : void {
      ...
      for (var i:int = 0; i < 7; i++) if (i != 6) this.color_btns[i].toggled = false;
